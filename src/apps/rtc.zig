...
fn drawTime(dt: rtc.DateTime) Error!void {
    try display.epd.setMode(display.epd.FASTEST);

    try display.fillRect(kTimeRect.x, kTimeRect.y, kTimeRect.w, kTimeRect.h, display.colors.WHITE);
    try display.text.setDatum(.middle_right);
    ...
    buf[len] = 0;
    try display.text.drawCstr(buf[0..len :0], 218, 200);
}

fn drawDate(dt: rtc.DateTime) Error!void {
    try display.epd.setMode(display.epd.FASTEST);

    try display.fillRect(kDateRect.x, kDateRect.y, kDateRect.w, kDateRect.h, display.colors.WHITE);
    try display.text.setDatum(.middle_right);
    try display.text.setColor(display.colors.BLACK, null);
    try fonts.use(.Montserrat36);
    ...
    const len = @min(s.len, buf.len - 1);
    buf[len] = 0;
    try display.text.drawCstr(buf[0..len :0], 218, 260);
}
...
pub const AppRtc = struct {
    ...
    pub fn update(self: *AppRtc, now_ms: i32, refresh: bool, tap: ?Tap) Error!void {
        try self.updateDateTime(now_ms, refresh);
        try self.updateSleepAndWakeUpButton(tap);
    }

    fn updateDateTime(self: *AppRtc, now_ms: i32, refresh: bool) Error!void {
        ...
        const dt = rtc.getDatetime() catch return;
        ...
        if (any) {
            ...
            const h = max_y - min_y;
            if (w > 0 and h > 0) {
                try display.updateRect(min_x, min_y, w, h);
            }
        }
    }

    fn updateSleepAndWakeUpButton(_: *AppRtc, tap: ?Tap) Error!void {
        if (tap) |t| {
            if (!kSleepButton.containsClick(t.x, t.y)) return;
            ...
            _ = rtc.clearIrq() catch {};
            _ = rtc.setAlarmIrq(16) catch {};
            try power.off();
        }
    }
};
