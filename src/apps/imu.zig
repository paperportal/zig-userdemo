...
pub const AppImu = struct {
    ...
    pub fn update(self: *AppImu, now_ms: i32, refresh: bool) Error!void {
        ...
        _ = imu.update() catch {};
        const accel = imu.get_accel() catch imu.Vec3{ .x = 0, .y = 0, .z = 0 };
        const gyro = imu.get_gyro() catch imu.Vec3{ .x = 0, .y = 0, .z = 0 };
        ...
        try display.fill_rect(kLeftRect.x, kLeftRect.y, kLeftRect.w, kLeftRect.h, display.colors.WHITE);
        try display.fill_rect(kRightRect.x, kRightRect.y, kRightRect.w, kRightRect.h, display.colors.WHITE);
        ...
        buf[len] = 0;
        try display.text.draw_cstr(buf[0..len :0], 24, 420);

        s = std.fmt.bufPrint(buf[0..], "AY: {d:.1}", .{accel.y}) catch buf[0..0];
        len = @min(s.len, buf.len - 1);
        buf[len] = 0;
        try display.text.draw_cstr(buf[0..len :0], 24, 458);

        s = std.fmt.bufPrint(buf[0..], "AZ: {d:.1}", .{accel.z}) catch buf[0..0];
        len = @min(s.len, buf.len - 1);
        buf[len] = 0;
        try display.text.draw_cstr(buf[0..len :0], 24, 496);

        s = std.fmt.bufPrint(buf[0..], "GX: {d:.1}", .{gyro.x}) catch buf[0..0];
        len = @min(s.len, buf.len - 1);
        buf[len] = 0;
        try display.text.draw_cstr(buf[0..len :0], 122, 420);

        s = std.fmt.bufPrint(buf[0..], "GY: {d:.1}", .{gyro.y}) catch buf[0..0];
        len = @min(s.len, buf.len - 1);
        buf[len] = 0;
        try display.text.draw_cstr(buf[0..len :0], 122, 458);

        s = std.fmt.bufPrint(buf[0..], "GZ: {d:.1}", .{gyro.z}) catch buf[0..0];
        len = @min(s.len, buf.len - 1);
        buf[len] = 0;
        try display.text.draw_cstr(buf[0..len :0], 122, 496);

        // Update both rectangles at once (they are adjacent).
        try display.update_rect(kLeftRect.x, kLeftRect.y, kLeftRect.w + kRightRect.w, kLeftRect.h);
